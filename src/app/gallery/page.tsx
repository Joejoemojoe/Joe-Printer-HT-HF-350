      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
        {images.map(img => (
          <div key={img.src} className="group relative border border-border rounded-md overflow-hidden bg-surface">
            <Image src={`${img.src.startsWith('/') ? basePath + img.src : img.src}`} alt={img.alt} width={400} height={260} className="object-cover w-full h-48 group-hover:scale-105 transition" />
